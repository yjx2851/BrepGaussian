...
#define MAX_FEATURE_DEGREE 20 // Maximum feature dimension supported
	...
	bool prefiltered)
{
	...
	// Compute extra_features2D
	for (int ch = 0; ch < feature_degree; ch++) {
		extra_features2D[idx * feature_degree + ch] = extra_features[idx * feature_degree + ch];
	}
	...
	float* __restrict__ out_others)
{
	...
	__shared__ float collected_extra_features2D[BLOCK_SIZE * MAX_FEATURE_DEGREE];
