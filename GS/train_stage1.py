...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint,feature_tag):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        render_pkg = render(viewpoint_cam, gaussians, pipe, background,feature_tag=feature_tag)
        image, segment_image, _, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["segment"], render_pkg["extra_features"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]

        gt_image = viewpoint_cam.original_image.cuda()
        gt_corner = viewpoint_cam.original_corner.cuda()
        ...
        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)) + 0.1 * edge_loss
        # + 0.1 * mask_loss
